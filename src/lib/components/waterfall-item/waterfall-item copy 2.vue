<template>
  ...
  <view :class="waterfallItemClass" :style="waterfallItemStyle">
    ...
    <!-- 正常内容 -->
    <slot
      v-if="!item.showErrorImage && !item.showErrorTip && !item.isEmpty"
      :on-load="onLoad"
    ...
    <!-- 错误图片模式 -->
    <view
      v-else-if="item.showErrorImage"
      class="sar-waterfall-item__error-image"
    >
      <image
        :src="props.errorImageSrc"
        mode="aspectFill"
        class="sar-waterfall-item__error-img"
      />
    </view>

    <!-- 占位符模式 -->
    <view v-else-if="item.isEmpty" class="sar-waterfall-item__placeholder">
      <text class="sar-waterfall-item__placeholder-text">加载失败</text>
    </view>
  </view>
</template>
...
const props = withDefaults(defineProps<WaterfallItemProps>(), {
  ...
  fallbackHeight: 150,
  errorImageSrc: '/static/error-placeholder.png',
})
...
const handleLoadFailure = async () => {
  switch (props.failureMode) {
    case 'error-image':
      item.showErrorImage = true
      item.height = props.fallbackHeight
      break
    case 'hide':
      item.visible = false
      item.height = 0
  ...
  console.log('item加载失败了，使用模式:', props.failureMode, item)
}
...
const item = shallowReactive<WaterfallItemInfo>({
  ...
  refreshImage: async () => {
    // 重新加载图片
    item.loaded = false
    item.showErrorImage = false
    item.showErrorTip = false
    item.isEmpty = false
    itemId.value = uniqid()
    // await item.beforeReflow()
  },
...
const waterfallItemClass = computed(() => {
  return classNames(
    ...
    bem.m('error-image', item.showErrorImage), // 错误图片状态
    bem.m('retry-tip', item.showErrorTip), // 重试提示状态
    bem.m('placeholder', item.isEmpty), // 占位符状态
